...
export function configureModerationForAccount(
  bundle: ModerationSession | LegacyModerationAgent,
  account: SessionAccount,
) {
  // This global mutation is *only* OK because this code is only relevant for testing.
  // Don't add any other global behavior here!
  switchToBskyAppLabeler()
  if ('appviewClient' in bundle && IS_TEST_USER(account.handle)) {
  ...
  if ('appviewClient' in bundle && labelerDids) {
    applyLabelersToClient(bundle.appviewClient, labelerDids)
    applyLabelersToClient(bundle.chatClient, labelerDids)
  } else if (!('appviewClient' in bundle)) {
    bundle.configureLabelers(labelerDids ?? [])
  } else {
