...
import {type GenerationStyle, type MaterialYouPalette} from '../Types'
import Color from '../Utils/Color'
import Shades from './Shades'

type StyleType = Record<
  GenerationStyle,
  Record<'a1' | 'a2' | 'a3' | 'n1' | 'n2', [number, number]>
>
...
export default class Palette {
  ...
  static getHueRotation(
    sourceHue: number,
    hueAndRotations: Array<[number, number]>,
  ): number {
    ...
    for (let i = 0; i < hueAndRotations.length - 1; i++) {
      ...
      if (thisHue <= sanitizedSourceHue && sanitizedSourceHue < nextHue) {
        return Palette.wrapDegreesDouble(
          sanitizedSourceHue + hueAndRotations[i][1],
        )
      }
    }
  ...
  static generate(
    ...
    const numberToHexColor = (colorNumber: number) => {
      return (
        '#' +
        (colorNumber & 0xffffff).toString(16).padStart(6, '0').toUpperCase()
      )
    }
